...
class HomeController extends GetxController with GetTickerProviderStateMixin {
  ...
  /// Page controller
  late PageController appBarTab;

  /// App Bar Tab controller
  late PageController homeTab;

  /// Dashboard tab controller
  late PageController webTab;

  /// Dashboard tab controller
  late PageController itemTab;
  ...
  bool? checkVisibility({required VisibilityEnum check}) {
    Map<VisibilityEnum, bool> visibilityCase = {
      ...
      VisibilityEnum.invisbleHomeIcon:
          (position.value > 0.2 && position.value <= 0.8),
    };
  ...
  Function homeFunctions({required HomeFunctions action}) {
    Map<HomeFunctions, void Function()> actions = {
      ...
      HomeFunctions.backIcon: () {
        homeFunctions(action: HomeFunctions.discoverBar)();
      },

      /// Click forward icon in app bar
      HomeFunctions.forwardIcon: () {
        homeFunctions(action: HomeFunctions.feedBar)();
      },

      /// Click search icon in tab bar
      HomeFunctions.search: () {
      ...
      HomeFunctions.discoverBar: () {
        homeTab.animateToPage(0,
            duration: durationLong, curve: Curves.easeInOut);
        appBarTab.animateToPage(0,
            duration: durationLong, curve: Curves.easeInOut);
      ...
      HomeFunctions.feedBar: () {
        homeTab.animateToPage(1,
            duration: durationLong, curve: Curves.easeInOut);
        appBarTab.animateToPage(1,
            duration: durationLong, curve: Curves.easeInOut);
      },
